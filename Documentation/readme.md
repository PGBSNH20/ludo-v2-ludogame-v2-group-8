...
### Sendgrid
Vi har skapat en konto på SendGrid och sen skapade en API key som vi skulle anvanda för att skicka mejl från spelet till en annar person för att utmanda de. Sendan har vi skapat en Controller som har en post metod som tar emot två input paramterar FromEmeil och ToEmail. Vi har updatert kod snippet som vi tog från SendGrid dokumentation.
...
För att testa att den skickar mejl vi har startade LudoGameApi projektet. Vi kunde testa sendEmail end-point och det gick och skicka mejl.
